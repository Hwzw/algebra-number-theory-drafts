...
function. No arithmetic condition on the edge lengths is imposed.
...
applications of the same result.
...
\clearpage\section{6. Consequences and limits}\label{consequences-and-limits}
...
Our conclusions concern unweighted finite metric graphs with the stated
Dirichlet and Kirchhoff conditions. They do not provide a heat-trace
bound, pointwise heat-kernel domination, or a theorem for arbitrary
vertex couplings. We prove no quantitative stability estimate. A useful
next problem is to quantify the loss in (2) in terms of geometric
distance from a path, with explicit control of the time scale and of
edges whose lengths tend to zero. Such a statement requires additional
work beyond the qualitative rigidity proved here.
