...
Theorems 1, 4 and 5 leave the full all-exponent form of [NS, Conjecture 4.14] unresolved. For each order, Theorem 5 proves an open interval about two, while Theorem 4 proves the constant identity and coincidence at the measure endpoint. Symmetry and an explicit classification of the maximizing knot configurations at p=1 remain open here. Further continuation requires excluding competing maximizers or finding a counterexample; the compactness argument alone does not do that.
