...
source=re.sub(r'^(#{2,6}) ', lambda m: m.group(1)[1:]+' ', source, flags=re.M)
body=subprocess.check_output([executable('pandoc'),'-f','markdown+tex_math_dollars-smart',
                              '-t','latex'],input=source,text=True)
body=body.replace(r'\section{',r'\Needspace{9\baselineskip}\section{')
body=re.sub(r'(?=\\textbf\{(?:Theorem|Lemma|Proposition|Corollary) )',
