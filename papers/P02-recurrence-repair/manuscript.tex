...
\title{Squarefree repair factors and optimal exponents for linear recurrences}
\author{}
\date{September 8, 2026}
\begin{document}
\maketitle
\begin{abstract}
An integer sequence satisfies the Dold congruences if its M\"obius transform at every positive integer $n$ is divisible by $n$. We determine the least multiplier that makes all entries of a power-indexed integer matrix sequence satisfy these congruences: under a finite-field exponent condition, it is the product of the primes at which the matrix has nonsemisimple reduction. Consequently, for a fixed separable integral recurrence polynomial $F$ of degree $d$ with nonzero constant term, the exact multiplier that works uniformly for every integral initialization indexed from $1$ is $\rad(|F(0)\disc(F)|)$ when sampling at $n^s$, where $s$ is a positive multiple of $\lcm(1,\ldots,d)$. For the fundamental Lucas sequence of nonzero discriminant $D$, we obtain the exact scalar repair factor $\rad(|D|)$ along every positive even power. We also show that the multiples of $\lcm(1,\ldots,d)$ are exactly the exponents admitting finite repair for every separable recurrence of order at most $d$ with nonzero constant term. The sufficiency proofs use Jordan decomposition over finite fields and lifting of congruences between commuting matrices; exponent necessity uses cyclic number fields and Chebotarev. These statements concern Dold divisibility; nonnegativity of orbit counts is a separate condition.
\end{abstract}
...
\]
The \emph{Dold congruences} require $n\mid b_n(a)$ for every $n\geq1$.
Write $\Fail(a)$ for the least positive integer $C$ such that $(Ca_n)$ satisfies these congruences, with $\Fail(a)=\infty$ if no such integer exists. We use this notation for signed sequences as well. It is a divisibility invariant: the condition $b_n(a)\geq0$, required to interpret $a_n$ as fixed-point counts of a map, is not part of its definition.

The repair phenomenon for power subsequences is exemplified by the Fibonacci sequence. Moss and Ward \cite{MW} established realizability after multiplication by $5$ along positive even powers and proposed a corresponding discriminant-scaled Lucas congruence. Luca and Ward \cite{LW}, Theorem~5, proved that conjecture. Rajs \cite{Rajs} investigated repair factors for general linear recurrences and their power subsequences. Here we work with integral matrices directly, avoiding the denominators of a Binet representation.

Set $L_d=\lcm(1,\ldots,d)$. For a nonzero integer $t$, $\rad(|t|)$ is the product of its distinct positive prime divisors, with $\rad(1)=1$.

\begin{theorem}[Uniform repair for a fixed recurrence]\label{thm:recurrence}
Let
\[
...
\begin{theorem}[Exact Lucas repair]\label{thm:lucas}
...
\[
 \Fail\bigl((U_{n^{2k}})_{n\geq1}\bigr)=\rad(|D|).
\]
If $D=0$, this failure invariant is infinite for every $k\geq1$.
\end{theorem}

\begin{theorem}[Optimal universal exponents]\label{thm:optimal-exponent}
Fix $d\geq1$ and $s\geq1$. The following are equivalent:
\begin{enumerate}
\item $L_d\mid s$;
\item for every integer sequence admitting a monic separable integral recurrence polynomial of degree at most $d$ with nonzero constant term, its subsequence $(u_{n^s})_{n\geq1}$ has finite Dold repair factor.
\end{enumerate}
...
\section{The matrix repair theorem}
...
\begin{theorem}[Exact entrywise matrix repair]\label{thm:matrix}
Let $A\in\Mat_N(\mathbb Z)$ be annihilated by a monic polynomial $F\in\mathbb Z[x]$ of degree $d\geq1$ with $\disc(F)\neq0$. Suppose $s\geq1$ satisfies:
\begin{enumerate}
\item for every prime $p$, the degree of every irreducible factor of $F$ modulo $p$ divides $s$;
\item $2^s\geq d$.
\end{enumerate}
...
\end{lemma}
\begin{proof}
Put $b_n=\sum_{e\mid n}\mu(n/e)x_e$. M\"obius inversion gives $x_n=\sum_{e\mid n}b_e$. Thus
\[
...
\[
 b_{mp^r}=\sum_{e\mid m}\mu(m/e)
      (x_{ep^r}-x_{ep^{r-1}}).
\]
The local hypothesis therefore makes $b_n$ divisible by the full power of every prime dividing $n$. Since $M$ is free abelian, it follows that $b_n\in nM$.
\end{proof}
...
\end{lemma}
\begin{proof}
Write $X=Y+p^aZ$. The commutativity of $X$ and $Y$ implies that $Z$ and $Y$ commute. In the binomial expansion of $(Y+p^aZ)^p-Y^p$, the terms with powers $Z^i$ for $1\leq i<p$ are divisible by $p^{ai+1}$, and the last term is divisible by $p^{ap}$. All are divisible by $p^{a+1}$, including when $p=2$ and $a=1$. Induction proves the assertion.
\end{proof}
...
For $r\geq2$, set $t=s(r-1)-h\geq0$. Applying Lemma~\ref{lem:lifting} to the commuting matrices in \eqref{eq:frobenius}, and then raising both sides to $m^s$, gives
...
At a semisimple prime take $h=0$. The same argument, now also for $r=1$, gives a modulus $p^{s(r-1)+1}$, which is at least $p^r$. Therefore no multiplier is needed there. If $p\nmid\disc(F)$, the polynomial $F$ modulo $p$ is squarefree and its annihilation of $A$ implies semisimplicity. Thus $\mathcal B(A)$ is finite and supported on the prime divisors of $\disc(F)$. Lemma~\ref{lem:local} proves sufficiency of $C(A)$.
...
Finally, if $L_d\mid s$, every irreducible factor degree divides $s$, and $s\geq d$ implies $2^s\geq d$.
\end{proof}

\section{Arbitrary initialization and uniform sharpness}
...
\]
Every irreducible factor of $xF$ modulo any prime has degree at most $d$, since the new factor $x$ has degree one. The exponent $s$ is divisible by all these degrees. Also $2^s\geq d+1$, including the case $d=1$. Theorem~\ref{thm:matrix} therefore implies that
\[
 \rad(|F(0)\disc(F)|)\,u_{n^s}
\]
satisfies the Dold congruences for every initial vector.

To prove universal sharpness, fix a prime $p\mid F(0)\disc(F)$ and reduce $T$ modulo $p$. Its minimal polynomial remains $F$ modulo $p$, because a companion matrix is cyclic over every field. Put
\[
 E=T^{p^s-1}-I.
\]
This is zero if and only if $F$ modulo $p$ divides $x^{p^s-1}-1$. Every root of $F$ modulo $p$ lies in $\mathbb F_{p^s}$. The roots of $x^{p^s-1}-1$ are exactly the nonzero elements of this field, each with multiplicity one. It follows that $E=0$ if and only if $F$ modulo $p$ is squarefree with nonzero constant term, equivalently if and only if $p\nmid F(0)\disc(F)$. Thus $E\neq0$.
...
\]
Thus every multiplier working for all initial values must be divisible by $p$. This holds for each prime in the proposed radical and proves minimality.
\end{proof}

\begin{example}[The constant term cannot be omitted]\label{ex:initialization}
Let $u_n=2^{n-1}+3^{n-1}$ for $n\geq1$. The recurrence polynomial is
\[
 F(x)=(x-2)(x-3)=x^2-5x+6,
\]
and $\disc(F)=1$. For every positive integer $s$, $u_{2^s}$ is odd while $u_1=2$, so the prime $2$ is required in every repair. Likewise, $u_{3^s}\equiv1\pmod3$, so $3$ is required. For every even $s$, Theorem~\ref{thm:recurrence} gives the factor $6$, which is therefore exact for this particular sequence.
\end{example}

\begin{remark}[Comparison with the printed general statement of Luca--Ward]
In \cite{LW}, Theorem~1(i), the stated sufficient multiplier is a multiple of $\lcm(\Delta(K),\Delta(F))$, with the recurrence initialized at $u_1,\ldots,u_d$, and the exponent required to be at least $[K:\mathbb Q]$ and divisible by the Galois-group exponent. Example~\ref{ex:initialization} has $K=\mathbb Q$ and both discriminants equal to $1$. Thus the printed statement allows multiplier $1$, contradicted by its prime-$2$ Dold congruence. This is a counterexample to that statement with its printed indexing conventions; we do not assert an official erratum or any conclusion about the other theorems of the article. The initialization argument above accounts explicitly for the extra prime support from $F(0)$.
\end{remark}

\section{The optimal universal sampling exponent}

\begin{proof}[Proof of Theorem~\ref{thm:optimal-exponent}]
If $L_d\mid s$, then $L_e\mid s$ for every $e\leq d$. Theorem~\ref{thm:recurrence} therefore gives a finite repair for every sequence in the stated class.

Conversely, suppose $L_d\nmid s$. There is an integer $f$, with $2\leq f\leq d$, such that $f\nmid s$. Choose a prime $q\equiv1\pmod f$, whose existence follows from Dirichlet's theorem; see \cite[Example~8.34]{Milne}. The Galois group of $\mathbb Q(\zeta_q)/\mathbb Q$ is cyclic of order $q-1$ by the standard cyclotomic description \cite[Section~6]{Milne}. Taking the fixed field of its subgroup of index $f$ produces a cyclic extension $K/\mathbb Q$ of degree $f$.
...
Fix a generator $\sigma$ of $\operatorname{Gal}(K/\mathbb Q)$. Chebotarev's theorem \cite[Theorem~8.31]{Milne}, applied to the conjugacy class $\{\sigma\}$ in this finite Galois extension, gives infinitely many unramified rational primes with Frobenius $\sigma$. Each is inert, since its residue degree is the order $f$ of its Frobenius. Exclude also the finitely many prime divisors of $[\mathcal O_K:\mathbb Z[\alpha]]$. At every remaining prime $p$,
\[
...
\[
 p\mid C(u_{p^s}-u_1).
\]
Each of infinitely many distinct primes must therefore divide $C$, which is impossible for a positive integer $C$. This proves necessity.
\end{proof}

\begin{remark}
The nonzero-constant-term restriction in Theorem~\ref{thm:optimal-exponent} is essential with initialization at $n=1$. The sequence $u_1=1$ and $u_n=0$ for $n>1$ is annihilated by the separable polynomial $x$ but has infinite repair factor along every positive power.
\end{remark}

\section{The exact Lucas factor}

\begin{proof}[Proof of Theorem~\ref{thm:lucas}]
Set
\[
...
If $D\neq0$, the polynomial $F(x)=x^2-Px+Q$ is separable. Every positive even exponent $s=2k$ meets the hypotheses of Theorem~\ref{thm:matrix}. That theorem proves sufficiency of $\rad(|D|)$.
...
If $D=0$, the same argument applies to every prime, precluding any positive finite repair multiplier. It also covers $P=Q=0$ directly.
\end{proof}
...
\begin{remark}[A stronger quadratic congruence]
...
\]
Here the exponent furnished by \eqref{eq:localbound} is $2r-2\geq r$, since one may take $h\leq1$. This statement does not require a nonzero discriminant. At discriminant primes the scalar obstruction is consequently already visible at the first local level.
\end{remark}

\section{Relation to previous bounds and verification}

Theorem~\ref{thm:lucas} sharpens the sufficient Lucas discriminant factor in \cite{MW,LW} to an exact radical. The matrix theorem explains this through nonsemisimple reduction. Rajs \cite{Rajs}, Theorem~4 and its proof, retains a denominator-clearing factor $N$ in addition to a radical for quadratic square subsequences. His Theorem~9 supplies a factor $r_d\Delta_U\rad(\Delta_K)$ for exponents divisible by the splitting-field degree. The bounds here remove higher prime powers and determine the uniform recurrence factor without constructing a splitting field.
...
\url{https://arxiv.org/html/2509.09847}.
\end{thebibliography}
\end{document}
